...
def fetch_fun_fact():
    ...
    if response.status_code == 200:
        ...
        if response_data:
            random_fact = random.choice(response_data)
            text_box.delete(1.0, "end")
            text_box.insert("end", random_fact['facts'])

            fact_id = random_fact['fact_id']
            requests.patch(f'http://localhost:8000/facts/update?fact_id={fact_id}')
        else:
            text_box.delete(1.0, "end")
            text_box.insert("end", "Keine passenden Fun Facts gefunden.")
    else:
        text_box.delete(1.0, "end")
        text_box.insert("end", "Keine Fun Facts mit diesem Status gefunden.")
